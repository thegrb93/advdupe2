...
if (SERVER) then
    ...
    local function PlayerCanDupeTool(ply, ent)
        ...
        local trace = WireLib and WireLib.dummytrace(ent) or {
            Entity = ent
        }
    ...
    function TOOL:RightClick(trace)
        ...
        elseif trace.HitNonWorld then
            ...
            if not duplicator.IsAllowed(trace.Entity:GetClass()) then return false end
    ...
    function TOOL:Reload(trace)
        ...
        if (trace.Entity:GetClass() == "gmod_contr_spawner") then
            ...
            if (not delay) then
                delay = GetConVar("AdvDupe2_MinContraptionSpawnDelay"):GetInt() or 0.2
            else
                if (not game.SinglePlayer()) then
                    min = GetConVar("AdvDupe2_MinContraptionSpawnDelay"):GetInt() or 0.2
            ...
            if (not undo_delay) then
                undo_delay = GetConVar("AdvDupe2_MinContraptionUndoDelay"):GetInt()
            else
                if (not game.SinglePlayer()) then
                    min = GetConVar("AdvDupe2_MinContraptionUndoDelay"):GetInt() or 0.1
                    max = GetConVar("AdvDupe2_MaxContraptionUndoDelay"):GetInt() or 60
    ...
    function MakeContraptionSpawner(ply, Pos, Ang, HeadEnt, EntityTable, ConstraintTable, delay, undo_delay, model, key, undo_key, disgrav, disdrag, addvel, hideprops)
        ...
        if (not game.SinglePlayer()) then
            if (table.Count(EntityTable) > GetConVar("AdvDupe2_MaxContraptionEntities"):GetInt()) then
                AdvDupe2.Notify(ply, "Contraption Spawner exceeds the maximum amount of " .. GetConVar("AdvDupe2_MaxContraptionEntities"):GetInt() .. " entities for a spawner!", NOTIFY_ERROR)

                return false
            end

            if (#ConstraintTable > GetConVar("AdvDupe2_MaxContraptionConstraints"):GetInt()) then
                AdvDupe2.Notify(ply, "Contraption Spawner exceeds the maximum amount of " .. GetConVar("AdvDupe2_MaxContraptionConstraints"):GetInt() .. " constraints for a spawner!", NOTIFY_ERROR)
    ...
    net.Receive("AdvDupe2_CanAutoSave", function(len, ply, len2)
        ...
        if (game.SinglePlayer()) then
            ...
            if (time < GetConVar("AdvDupe2_AreaAutoSaveTime"):GetInt()) then
                time = GetConVar("AdvDupe2_AreaAutoSaveTime"):GetInt()
            end
        end

        AdvDupe2.Notify(ply, "Your area will be auto saved every " .. (time * 60) .. " seconds.")
        local name = "AdvDupe2_AutoSave_" .. ply:SteamID64()
        ...
        timer.Create(name, time * 60, 0, function()
            ...
            ply.AdvDupe2.FileMod = CurTime() + GetConVar("AdvDupe2_FileModificationDelay"):GetInt()
        end)
    ...
    concommand.Add("AdvDupe2_RemoveAutoSave", function(ply, cmd, args)
        timer.Remove("AdvDupe2_AutoSave_" .. ply:SteamID64())
    end)
...
if (CLIENT) then
    ...
    function TOOL:RightClick(trace)
        if (self:GetOwner():KeyDown(IN_SPEED) and not self:GetOwner():KeyDown(IN_WALK)) then return false end
    ...
    function TOOL:Reload(trace)
        if (trace and (AdvDupe2.HeadGhost or self:GetStage() == 1)) then return true end
    ...
    local function BuildCPanel(CPanel)
        ...
        Check:SetValue(GetConVar("advdupe2_sort_constraints"):GetInt())
        ...
        CategoryContent1:SetPaintBackground(false)
        ...
        CategoryContent2:SetPaintBackground(false)
        ...
        CategoryContent3:SetPaintBackground(false)
        ...
        if (game.SinglePlayer()) then
            ...
            local min = GetConVar("AdvDupe2_MinContraptionSpawnDelay"):GetInt() or 0.2
        ...
        if (game.SinglePlayer()) then
            ...
            local min = GetConVar("AdvDupe2_MinContraptionUndoDelay"):GetInt() or 10
            local max = GetConVar("AdvDupe2_MaxContraptionUndoDelay"):GetInt() or 60
        ...
        CategoryContent4:SetPaintBackground(false)
        ...
        NumSlider.Label:SetDark(true)
        NumSlider:SetMin(GetConVar("AdvDupe2_AreaAutoSaveTime"):GetInt())
        NumSlider:SetMax(30)
        NumSlider:SetDecimals(0)
        ...
        CategoryContent5:SetPaintBackground(false)
        ...
        if (LocalPlayer():IsAdmin()) then
            ...
            CategoryContent6:SetPaintBackground(false)
