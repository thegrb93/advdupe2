...
decode_types_v2 = {
    ...
    [4] = function() return false end, -- false boolean
    [5] = function() return true end, -- true boolean
    [6] = function()
        m = find(str, "\1", pos)
...
local function lzwDecode(encoded)
    ...
    while pos <= encoded_length do
        ...
        --now we know it's a length indicator for a multibyte index
...
local function deserializeChunk(chunk)
    ...
    elseif ctype == 84 then
        ...
        return t
    elseif ctype == 78 then
        return tonumber(val)
    elseif ctype == 83 then
        return gsub(sub(val, 2, -2), "�", ";")
    elseif ctype == 66 then
        return val == "t"
    elseif ctype == 80 then
        return 1
    end
        ...
        for id, tbls in pairs(subtables) do
            for _, tbl in pairs(tbls) do
                if not tables[id] then
                    error("attempt to reference a nonexistent table")
                end
...
versions[2] = function(encodedDupe)
    ...
    return deserialize_v2(lzwDecode(huffmanDecode(invEscapeSub(dupestring)))), info
end
...
versions[1] = function(encodedDupe)
    ...
    return deserialize_v1(lzwDecode(huffmanDecode(invEscapeSub(dupestring)))), info
end
