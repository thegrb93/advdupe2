...
local function SaveFile(ply, cmd, args)
    if (not ply.AdvDupe2 or not ply.AdvDupe2.Entities or table.Count(ply.AdvDupe2.Entities) == 0) then
    ...
    ply.AdvDupe2.FileMod = CurTime() + GetConVar("AdvDupe2_FileModificationDelay"):GetInt() + 2
...
function AdvDupe2.LoadDupe(ply, success, dupe, info, moreinfo)
    ...
    if not game.SinglePlayer() and GetConVar("AdvDupe2_MaxConstraints"):GetInt() ~= 0 and #dupe["Constraints"] > GetConVar("AdvDupe2_MaxConstraints"):GetInt() then
        AdvDupe2.Notify(ply, "Amount of constraints is greater than " .. GetConVar("AdvDupe2_MaxConstraints"):GetInt(), NOTIFY_ERROR)
