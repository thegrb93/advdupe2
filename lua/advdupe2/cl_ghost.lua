...
local function MakeGhostsFromTable(EntTable)
    ...
    if (not EntTable.Model or EntTable.Model == "" or EntTable.Model[#EntTable.Model - 3] ~= ".") then
...
function AdvDupe2.UpdateGhosts(force)
    ...
    if (GetConVar("advdupe2_original_origin"):GetBool()) then
        ...
        if (GetConVar("advdupe2_offset_world"):GetBool()) then
            headangle = Angle(0, 0, 0)
        end

        trace.HitPos.Z = trace.HitPos.Z + math.Clamp(AdvDupe2.HeadZPos + GetConVar("advdupe2_offset_z"):GetInt() or 0, -16000, 16000)
        origin = trace.HitPos
        offsetang = Angle(math.Clamp(GetConVar("advdupe2_offset_pitch"):GetInt() or 0, -180, 180), math.Clamp(GetConVar("advdupe2_offset_yaw"):GetInt() or 0, -180, 180), math.Clamp(GetConVar("advdupe2_offset_roll"):GetInt() or 0, -180, 180))
        headpos, headang = LocalToWorld(AdvDupe2.HeadOffset, headangle, origin, offsetang)
    end

    if math.abs(Lheadpos.x - headpos.x) > 0.01 or math.abs(Lheadpos.y - headpos.y) > 0.01 or math.abs(Lheadpos.z - headpos.z) > 0.01 or math.abs(Lheadang.p - headang.p) > 0.01 or math.abs(Lheadang.y - headang.y) > 0.01 or math.abs(Lheadang.r - headang.r) > 0.01 or force then
        Lheadpos = headpos
        Lheadang = headang
