...
local function CopyEntTable(Ent, Offset)
    ...
    if EntityClass then
        ...
        for iNumber, Key in pairs(EntityClass.Args) do
            ...
            if (Key == "Pos" or Key == "Model" or Key == "Ang" or Key == "Angle" or Key == "ang" or Key == "angle" or Key == "pos" or Key == "position" or Key == "model") then continue end
    ...
    if (Ent:HasBoneManipulations()) then
        ...
        for i = 0, Ent:GetBoneCount() do
            ...
            if (s ~= Vector(1, 1, 1)) then
                t['s'] = s
            end

            if (a ~= Angle(0, 0, 0)) then
                t['a'] = a
            end

            if (p ~= Vector(0, 0, 0)) then
                t['p'] = p
            end
...
local function Copy(Ent, EntTable, ConstraintTable, Offset)
    ...
    do
        ...
        if IsValid(parent) then
            Copy(parent, EntTable, ConstraintTable, Offset)
        end
    ...
    for _, Constraint in pairs(Constraints) do
        ...
        if (not CopyOutside) then
            ...
            for k, v in pairs(EntTab) do
                if (not Entities[v:EntIndex()]) then
                    add = false
                end
            end
...
local function CreateEntityFromTable(EntTable, Player)
    ...
    if (not GENERIC) then
        ...
        for iNumber, Key in pairs(EntityClass.Args) do
            ...
            if (Key == "ang" or Key == "Ang" or Key == "angle") then
                Key = "Angle"
            end
        ...
        elseif IsAllowed(Player, EntTable.Class, EntityClass) then
            ...
            if (not EntTable.BuildDupeInfo.IsVehicle and not EntTable.BuildDupeInfo.IsNPC and EntTable.Class ~= "prop_ragdoll") then
    ...
    if (status ~= false and IsValid(valid)) then
        if (sent) then
            ...
            if (EntTable.Skin) then
                valid:SetSkin(EntTable.Skin)
            end

            if (EntTable.BodyG) then
                RestoreBodyGroups(valid, EntTable.BodyG)
            end
...
local function AdvDupe2_Spawn()
    ...
    if (Queue.Entity) then
        ...
        if Ent then
            ...
            if (not Queue.DisableProtection) then
                Ent:SetNotSolid(true)
            end
        ...
        if (Queue.Current > #Queue.ConstraintList) then
            ...
            local undos = undo.GetTable()[Queue.Player:SteamID64()]
            local str = "AdvDupe2_" .. Queue.Player:SteamID64()
...
function AdvDupe2.InitPastingQueue(Player, PositionOffset, AngleOffset, OrigPos, Constrs, Parenting, DisableParents, DisableProtection)
    ...
    undo.Create("AdvDupe2_" .. Player:SteamID64())
    undo.SetPlayer(Player)
