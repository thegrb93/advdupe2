...
function ENT:Initialize()
    ...
    if WireLib then
        self.Inputs = Wire_CreateInputs(self, {"Spawn", "Undo"})
...
function ENT:SetOptions(ply, delay, undo_delay, key, undo_key, disgrav, disdrag, addvel, hideprops)
    ...
    self.CreateKey = numpad.OnDown(ply, self.key, "ContrSpawnerCreate", self, true)
    self.UndoKey = numpad.OnDown(ply, self.undo_key, "ContrSpawnerUndo", self, true)
...
function ENT:AddGhosts()
    ...
    for EntIndex, v in pairs(selfTable) do
        if (EntIndex ~= self.HeadEnt) then
            if (selfTable[EntIndex].Class == "gmod_contr_spawner") then
                selfTable[EntIndex] = nil
                continue
            end

            EntTable = table.Copy(selfTable[EntIndex])
            ...
            if (EntTable.Model == nil or not util.IsValidModel(EntTable.Model)) then
...
function ENT:GetCreationDelay()
    return self.delay
end

function ENT:GetDeletionDelay()
    return self.undo_delay
end
...
function ENT:SetDupeInfo(HeadEnt, EntityTable, ConstraintTable)
    ...
    selfTable = EntityTable
    self.ConstraintTable = ConstraintTable

    if (not self.DupeAngle) then
        self.DupeAngle = self:GetAngles()
    end

    if (not self.EntAngle) then
        self.EntAngle = EntityTable[HeadEnt].PhysicsObjects[0].Angle
    end

    if (not self.Offset) then
        self.Offset = selfTable[HeadEnt].PhysicsObjects[0].Pos
    end
...
function ENT:DoSpawn(ply)
    ...
    local Ents, Constrs = AdvDupe2.duplicator.Paste(ply, selfTable, self.ConstraintTable, nil, nil, Vector(0, 0, 0), true)
    ...
    for k, ent in pairs(Ents) do
        ...
        if IsValid(phys) then
            ...
            if (self.DisableGravity == 1) then
                phys:EnableGravity(false)
            end

            if (self.DisableDrag == 1) then
                phys:EnableDrag(false)
            end

            phys:EnableMotion(true)

            if (ent.SetForce) then
                ent.SetForce(ent, ent.force, ent.mul)
            end
...
function ENT:ShowOutput()
    ...
    self:SetOverlayText("Spawn Delay: " .. tostring(self:GetCreationDelay()) .. "\nUndo Delay: " .. tostring(self:GetDeletionDelay()) .. text)
end
